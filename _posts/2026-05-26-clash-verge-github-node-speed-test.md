...
GitHub 官方面向受限网络和自托管 Runner 的文档也分别列出了 `codeload.github.com`、`objects.githubusercontent.com` 和 `release-assets.githubusercontent.com` 等用途。不过官方同时提醒：域名清单可能变化，也不保证覆盖所有访问链路。因此这里的六个目标是实用测试集，不是永久完整的 GitHub 域名表。
...
下面两个 GitHub 官方 URL 在 2026-07-13 核验时存在，可作为示例：
...
固定版本的 Release URL 未来可能被移除或替换。实际测试前应先打开 [GitHub CLI Releases](https://github.com/cli/cli/releases) 选择当前存在的资产，不要把 v2.92.0 当成永久测试地址。
...
- [GitHub 自托管 Runner 所需域名](https://docs.github.com/en/actions/reference/runners/self-hosted-runners#accessible-domains-by-function)
- [从受限网络访问 GitHub](https://docs.github.com/en/enterprise-cloud@latest/get-started/using-github/allowing-access-to-githubs-services-from-a-restricted-network)
