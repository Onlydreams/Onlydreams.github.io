...
- 新建专题通常应已有至少三篇语义一致的文章，或存在明确的连续写作计划；不要为单篇文章创建专题。同一篇文章可以属于多个专题，`series_order` 必须为每个专题 key 提供独立顺序。
...
- 新增或实质更新公开文章后，应在本地生成页检查独立 `description`、canonical、`BlogPosting` JSON-LD、作者、日期、代表图片和 sitemap 条目。只完成本地构建检查时，不得声称已经完成线上抓取、富媒体结果或索引验证。
- 面向搜索引擎和 AI Agent 的文章应保证核心正文无需 JavaScript 即可读取，并使用可独立理解的首段摘要、清晰标题层级、适用故障指纹、可观察事实与未确认边界、可复制命令和官方参考资料。`robots.txt` 未屏蔽目标爬虫只能证明仓库配置允许抓取，CDN、WAF 和真实爬虫访问仍需线上验证。
- 文章图片统一放在 `assets/images/YYYY-MM-DD-slug/`；截图应裁掉无关界面并检查用户名、路径、设备实例 ID、账号和其他设备指纹，同时添加准确的 `alt` 文本。文章专属封面应与正文直接相关、控制文件体积，并通过 front matter 的 `image` 提供给 OG 和结构化数据；截图只能证明画面中直接可见的状态。
